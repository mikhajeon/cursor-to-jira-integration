#!/usr/bin/env node
/**
 * US-3.1 (DPH-243): Put all subtasks assigned to Ben Schaumkel into [3.1-UX] Tasks
 * as a numbered list in the description, then delete those subtasks.
 *
 * Run from project root: node scripts/consolidate-us31-ux-by-assignee.mjs
 * Dry run: node scripts/consolidate-us31-ux-by-assignee.mjs --dry-run
 */
...
const PARENT = 'DPH-243';
const UX_SUMMARY = '[3.1-UX] Tasks';
...
async function main() {
  ...
  if (dryRun) {
    console.log(existingUx ? `[dry-run] Would update ${existingUx.key}` : '[dry-run] Would create [3.1-UX] Tasks');
