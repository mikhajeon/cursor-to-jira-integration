#!/usr/bin/env node
/**
 * US-3.1 (DPH-243): Put all subtask names with [UX], [DEV FE], [DEV BE] into
 * [3.1-UX] Tasks, [3.1-FE] Tasks, [3.1-BE] Tasks as numbered lists in their
 * descriptions, then delete the original subtasks.
 *
 * Run from project root: node scripts/consolidate-us31-by-discipline.mjs
 * Dry run: node scripts/consolidate-us31-by-discipline.mjs --dry-run
 */
...
const PARENT = 'DPH-243'; // US-3.1 Upload Project in Various Formats

const DISCIPLINES = [
  { key: 'UX', summary: '[3.1-UX] Tasks', pattern: /^\[UX\]/i },
  { key: 'FE', summary: '[3.1-FE] Tasks', pattern: /^\[DEV FE\]/i },
  { key: 'BE', summary: '[3.1-BE] Tasks', pattern: /^\[(DEV )?BE\]/i }, // [BE] or [DEV BE]
];
